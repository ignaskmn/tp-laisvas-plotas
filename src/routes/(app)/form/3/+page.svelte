<script lang="ts">
	...
	$: $form.firstName === '' && $form.phone === '' && $form.email === '' && $form.organization === ''
		? handleEmptied()
		: handleFilled();
	...
	async function insertData(formData: any) {
		...
		const activity =
			formData.activity != ('' || undefined || null) ? parseInt(formData.activity.value) : null;
...
<section class="layout">
	...
	<div class="input-container grow1">
		<Input name="organization" label="Organizacija" bind:value={$form.organization} />
	</div>
	<Button label="Siųsti užklausą" {isActive} onClick={() => insertData($form)} />
</section>
